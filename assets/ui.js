...
function config() {
  ...
  return {level:Math.max(1,Math.min(260,get('playerLevel'))), party, damage:Math.max(1,Math.min(1e12,Number(document.getElementById('playerDamage').value || DEFAULT_SETTINGS.playerDamage))),
...
function planningCells(r,identity,monster=false) {
  ...
  return `<td data-label="${labels.name}" class="identity-cell">${identity}</td>
    ...
    <td data-label="${labels.hp}">${numberCell(r.hp)}<span class="cell-note">${monster?'ต้องตี '+fmt(r.hitsPerKill)+' ครั้ง':'โจมตีเฉลี่ย '+fmt(r.hitsPerKill,1)+' ครั้ง/ตัว'}</span></td>
...
function monsterRows(map,c) {
  ...
  return rows.map(m=>{
    ...
    return `<tr class="monster-planning-row${m.damageWarning?' damage-warning':''}">${planningCells(m,identity,true)}</tr><tr id="monster-calculation-${m.index}" class="calculation-row" hidden><td colspan="6"><strong>${alt} · รายละเอียดการคำนวณ</strong>${details}${baseSourceLink}</td></tr>`;
  }).join('');
}
...
function readInputs() {
  return cleanSettings(Object.fromEntries(IDS.map(id=>{const el=document.getElementById(id);return [id,el.type==='checkbox'?el.checked:id==='playerDamage' && el.value===''?DEFAULT_SETTINGS.playerDamage:el.type==='number'||id.endsWith('Bonus')?Number(el.value):el.value];})),SPOTLIGHT_EVENTS);
}
