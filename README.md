...
## ปรับจากการตรวจรีวิว (12 กันยายน 2026)

- `assets/data/planning-context.js` เก็บชื่ออังกฤษและวิธีเข้าที่มีข้อมูลรองรับแยกจากตัวเลขมอนเดิม การค้นหาครอบคลุมชื่ออังกฤษ/aliases ที่เพิ่ม เช่น Magma, Oz Labyrinth, Magic Power Plant และ Frozen Scale โดยยังรักษาชื่อเล่นเดิม ไม่ได้อ้างว่าตรวจชื่อ/วิธีเข้าครบทุกแมพ
- ผู้เล่นยืนยันว่าพื้นที่ภายนอก EP19 ในภาพ memo ได้: `jor_back1/2/3` ใช้เกณฑ์เลเวล 1 ผ่าน Warp Portal พร้อมเงื่อนไขว่าต้องมีผู้เปิดวาร์ปให้ ค่า `min` ในข้อมูลเก่ายังรักษาไว้ แต่ `minimumEntryLevel()` และ `row()` ใช้เกณฑ์ที่อัปเดต วิธีนี้ไม่ปลดล็อก `jor_ab01/2` หรือดันอื่นโดยอนุมาน
- `jor_dun01` ใช้เส้นทางที่บันทึกไว้: วาร์ปไป `jor_back3` แล้วเข้าทางเชื่อม ยังไม่ระบุว่าวาร์ปตรงภายในได้ ส่วน Oz 2 ใช้ทางเชื่อมจาก Oz 1 โดยให้ตรวจปลายทางตรงในเมนูตั๋ว
- ตามข้อมูลผู้เล่น: `[Not For Sale] World Moving Ticket` แจกผ่านกิจกรรม/หน้าเว็บเป็นระยะ ผู้เล่นใหม่ซื้อ World Moving Ticket แบบปกติจาก Cash Shop ได้ ไม่ระบุราคาและไม่เหมาว่าตั๋วทุกชนิดมีปลายทางเหมือนกัน
- Modal แสดงวิธีเข้าและที่มาของฐาน No event แยกจาก Event EXP; `Main` หมายถึงเอกสารหลัก ไม่ใช่ตราว่ายืนยัน EXP/HP/จำนวนมอนในเกมครบทั้งแมพ
- วันที่ตรวจคลังกิจกรรมคือ **2026-09-12** แยกจากวันจบกิจกรรมล่าสุดในข้อมูล พ้นวันจบที่เวลาไทย 06:00 จะขึ้นว่ารอข้อมูลใหม่ ไม่ยืนยันว่าไม่มี event บนเซิร์ฟเวอร์จริง
- มือถือ/จอแคบยุบกลุ่มกิจกรรมและบัฟตอนเปิดครั้งแรก สถานะย่อยังแสดงกิจกรรม บัฟ และโบนัสเผ่าที่ใช้อยู่
- Auto ตรวจเวลาทุกนาทีแต่ render เฉพาะเมื่อกิจกรรมหรือสถานะหมดช่วงข้อมูลเปลี่ยน หากจำเป็นต้อง render รายละเอียดเดิมจะรักษาส่วนที่กาง มุมมองภาพ และ scroll ไว้ Reset คืนการเรียงทั้งแมพและมอนสเตอร์
- ปุ่มคัดลอกลิงก์บรรจุเฉพาะค่าที่กรอกใน URL fragment พร้อม version; ผู้รับเปิดแล้วใช้ค่าลิงก์ก่อน localStorage ตรวจและจำกัดค่าด้วยกติกาเดียวกับช่องกรอก ไม่ส่งลิงก์ให้บุคคลอื่นอัตโนมัติ หาก clipboard ใช้ไม่ได้จะแสดงช่องให้คัดลอกเอง Reset ลบค่าที่แชร์ออกจาก URL

ยังไม่เพิ่ม EXP/นาทีจาก damage ช่องเดียว ไม่ตั้งโบนัสรายวันจากกิจกรรมเก่าเป็นค่าปัจจุบัน และไม่ย้าย Range/EXP table ต่างแพตช์มาเป็นคำแนะนำตายตัว
